...
**三个角色：开发人员、Product Owner、Scrum Master（Scrum Team）**
...
- **检视**：Scrum 工件和实现商定目标的进展必须经常地和勤勉地检视，以便发现潜在的不良的差异或问题。
...
## 1.4 五个价值：承诺、专注、开放、尊重和勇气（Scrum 价值观）

## 1.5 三个角色：开发人员、PO、Scrum Master（Scrum Team）
...
<img src="./assets/1Scrum/image-20250220203405382.png" alt="image-20250220203405382" style="zoom:67%;"/>
...
## 1.7 三个工件：产品 Backlog、Sprint Backlog、增量
...
## 1.8 五个事件：Sprint、Sprint 计划会议、每日 Scrum 会议、Sprint 评审会议、Sprint 回顾会议
...
<img src="./assets/1Scrum/image-20250221100922099.png" alt="image-20250221100922099" style="zoom:67%;" align="left"/>

<img src="./assets/1Scrum/image-20250221100328304.png" alt="image-20250221100328304" style="zoom: 67%;" align="left"/>
...
- 工具：使用白板
  ...
  <img src="./assets/1Scrum/image-20250221100849689.png" alt="image-20250221100849689" style="zoom:67%;" />
...
**局限**：没有技术实践。可以使用极限编程技术实践：测试驱动开发、简单设计、重构、持续集成等等
...
Scrum 适用性：超越软件领域，适用于复杂创新工作



## 1.10 大规模敏捷 – SAFe

SAFe 定义了团队层、项目群（Program）层、解决方案层、组合（Portfolio）层等层级，每层都有相应的角色和事件，以协调大量Scrum团队的工作

在项目群层，引入了 Agile Release Train（ART）的概念，将5-12个团队组成一个发布“列车”，以 PI（Program Increment）为周期（通常5个 Sprint 左右）进行同步规划和交付。ART 有专门的 Release Train Engineer（类似超级 Scrum Master）和 Product Management（多产品负责人的协调）角色，来管理列车的协同

SAFe强调通过统一节奏和里程碑来协调多个团队。
比如所有团队按相同Sprint节奏工作，每隔一定Sprint统一在PI规划大会上协同计划下个PI目标。
还有Scrum of Scrums(各团队Scrum Master参加）和PO同步会等，解决跨团队的依赖。
当然SAFe也有争议，被一些敏捷倡导者批评为“过于繁重”或“披着敏捷外衣的瀑布”。批评者认为SAFe层级过多，引入大量新角色和流程，可能与敏捷初心相悖 .

## 1.11 LeSS（Large-Scale Scrum）

LeSS的核心理念是“Large-Scale Scrum is Scrum”，即大规模Scrum仍然是Scrum，不引入额外的复杂框架，而是通过扩展Scrum的基本原则和实践来适应多团队环境

LeSS提供了两种框架，分别适用于不同规模的团队：

- 基本LeSS（Basic LeSS）： 适用于2-8个团队（每个团队通常5-9人），共同开发一个产品
- LeSS Huge： 适用于8个以上团队，通常用于超大规模产品开发，可能涉及数百甚至数千人

基本LeSS的结构：

- 一个产品负责人（Product Owner）： 负责整个产品的愿景、优先级和产品待办事项列表（Product Backlog）。产品负责人需要与所有团队密切合作，确保所有团队对产品目标的理解一致
- 多个Scrum团队： 每个团队遵循标准的Scrum实践，包括迭代开发（Sprint）、每日站会（Daily Scrum）、冲刺计划（Sprint Planning）、冲刺评审（Sprint Review）和回顾会议（Retrospective）。团队是跨职能的，能够独立交付功能
- 一个产品待办事项列表： 所有团队共享同一个产品待办事项列表，确保工作的透明性和一致性
- 一个冲刺： 所有团队在同一时间段内进行冲刺，目标是交付一个集成的、可发布的增量（Increment）
